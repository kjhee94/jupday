...
<%
	...
	Notice notice = (Notice)request.getAttribute("notice");//데이터를 가져왔다.
	//notice 안에는 두가지 상황이 있다. 
	//1. notice 객체가 담겨있는 상황 (정상적으로 해당 글의 정보를 가져왔을때)
		...
		<div id="content">
			<div class="box-title">
         	...
         	<div class="box-notice">
	         	<div class="notice-top">
	         		<div class="box-subject">
	         			<p><%=notice.getNoticeTitle()%></p>
	         		</div>
	         		<div class="box-notice-info">
	         			<div class="box-gray">조회수</div>
	         			<span><%=notice.getNoticeHits()%></span>
	         		</div>
	         		<div class="box-notice-info">
	         			<div class="box-gray">작성일</div>
	         			<span><%=notice.getNoticeRegDate()%></span>
	         		</div>
	         	</div>
         	...
         	<div class="box-button">
         		<button class="btn-m">
	         		<a href="/views/notice/noticeNewsAllSelect.do?currentPage=<%=currentPage%>">목록</a>
	         	</button>
         	</div>
         			<%}else{%>
				<script>
				  alert("해당글이 존재 하지 않습니다. 다시 확인해주세요");
				  location.replace('/views/notice/noticeNewsAllSelect.do');
				</script>		
				<%} %>
