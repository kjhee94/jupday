...
public class AdminCrewFeedDAO<CrewNo> {
   ...
   public int totalCount(Connection conn)
   {
      ...
      String query = "SELECT COUNT(*) as count FROM CREW_FEED";
   ...
   public String getFeedPageNavi(Connection conn, int naviCountPerPage, int recordCountPerPage, int currentPage, int crewNo)
   {
      //전체 피드수
            ...
            if(startNavi!=1) {
               sb.append("<li><a href='/admin/crewFeedManageList.do?currentPage="+(startNavi-1)+"&crewNo="+crewNo+"'><i class='fas fa-chevron-left'></i></a></li>");
            }

            for(int i=startNavi; i<=endNavi; i++) {
               
               if(i==currentPage) {
                  sb.append("<li><a href='/admin/crewFeedManageList.do?currentPage="+i+"&crewNo="+crewNo+"' class='page_active'>"+i+"</a></li>");
               }else {
                  sb.append("<li><a href='/admin/crewFeedManageList.do?currentPage="+i+"&crewNo="+crewNo+"'>"+i+"</a></li>");
               }
            }

            if(endNavi!=pageTotalCount) {
               sb.append("<li><a href='/admin/crewFeedManageList.do?currentPage="+(endNavi+1)+"&crewNo="+crewNo+"'><i class='fas fa-chevron-right'></i></a></li>");
            }
            
            return sb.toString();
   }

   public int totalFeedCount(Connection conn, int crewNo)
   {
      PreparedStatement pstmt = null;
      ResultSet rset = null;
