...
/**
 * Servlet implementation class CrewDelYNChangeServlet
 */
@WebServlet("/admin/crewDelYNChange.do")
public class CrewDelYNChangeServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CrewDelYNChangeServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int crewNo = Integer.parseInt(request.getParameter("c_No"));
		char delYN = request.getParameter("c_Del_YN").charAt(0);
		
		if(delYN=='Y') delYN='N';
		else			delYN='Y';
		AdminCrewService adcService = new AdminCrewServiceImpl();
		int result = adcService.updateCrewDelYN(crewNo,delYN);
